...
def recover_password(app_n):
    cursor.execute(f"""
                    SELECT name, password FROM passwords WHERE name = '{app_n}'
                  ;""")


def edit_account(app_n, new_password):
    cursor.execute(f"""
                   UPDATE passwords SET password = '{new_password}' WHERE name = '{app_n}'
                   ;""")
